...
int RequestParam::parse_query_url(std::string &query_url) {
    ...
    DEBUG("start parse_query_url:%s", query_url.c_str());

    while(query_ss.good()) {
        std::string key_value;
        std::getline(query_ss, key_value, '&');
        DEBUG("get key_value:%s", key_value.c_str());
...
int RequestLine::parse_request_url_params() {
    ...
    DEBUG("start parse params which request_url:%s", request_url.c_str());
...
int ss_on_headers_complete(http_parser *p) {
    ...
    if (req->header_fields.size() != req->header_values.size()) {
        ERROR("header field size:%u != value size:%u",
    ...
    DEBUG("HEADERS COMPLETE! which field size:%u, value size:%u",
        req->header_fields.size(), req->header_values.size());
    return 0;
}
...
int Request::parse_request(const char *read_buffer, int read_size) {
    ...
    if (total_req_size > MAX_REQ_SIZE) {
        INFO("TOO BIG REQUEST WE WILL REFUSE IT!");
        return -1;
    }
    ...
    if (nparsed != read_size) {
        ...
        ERROR("parse request error! msg:%s", err_msg.c_str());
        return -1;
    }
    DEBUG("read from client: size:%d, content:%s", read_size, read_buffer);
...
int Response::gen_response(std::string &http_version, bool is_keepalive) {
	DEBUG("START gen_response code:%d, msg:%s", code_msg.status_code, code_msg.msg.c_str());
	...
	DEBUG("gen response context:%s", res_bytes.str().c_str());
	return 0;
}
