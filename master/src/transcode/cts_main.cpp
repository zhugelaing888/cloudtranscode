...
string CTS_Main::getVideoInfo(string srcframe,string typepath,const int num,const int ptime,const int pseg,string pixels,string &ttime)
{
	...
	for(int i=0;i<num;i++)
	{
		...
		cmd.append(" -y -ss ").append(str).append(" -i ").append(src_path).append(" -vframes 1 ").append(" -s ").append(pixels).append(" ").append(str_name).append(" 2>/dev/null");
		INFO("%s",cmd.c_str());
		system(cmd.c_str());
		int upload = 5;
		while(upload--)
		{		
				...
				sprintf(oss_str_name,"%s/%s%d.jpg",typepath.c_str(),str_addr.c_str(),i+1);
				if(access(str_name,0) == 0)
				{
					INFO("begin put object from file [%s]!",oss_str_name);
					if(g_oss_ftp->put_object_from_file(oss_str_name,str_name) == 0)
					{
						break;
					}
					sleep(5);
