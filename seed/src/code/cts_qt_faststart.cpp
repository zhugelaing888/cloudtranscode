...
int qt_faststart(const char *src,const char *dest)
{
    FILE *infile  = NULL;
    FILE *outfile = NULL;
    ...
    if (!strcmp(src, dest)) {
        ERROR("input and output files need to be different");
        return 1;
    }
    ...
    while (!feof(infile)) {
        ...
        if (atom_type == FTYP_ATOM) {
            ...
            if (!ftyp_atom) {
                ERROR("could not allocate %"PRIu64" bytes for ftyp atom",
                       atom_size);
                goto error_out;
            }
        ...
        if ((atom_type != FREE_ATOM) &&
            ...
            (atom_type != UUID_ATOM) &&
            (atom_type != FTYP_ATOM)) {
            INFO("encountered non-QT top-level atom (is this a QuickTime file?)");
            break;
        }
    ...
    if (atom_type != MOOV_ATOM) {
        INFO("last atom in file was not a moov atom");
        free(ftyp_atom);
        fclose(infile);
        return 0;
    }
    ...
    if (!moov_atom) {
        ERROR("could not allocate %"PRIu64" bytes for moov atom", atom_size);
        goto error_out;
    }
    ...
    for (i = 4; i < moov_atom_size - 4; i++) {
        ...
        if (atom_type == STCO_ATOM) {
            ...
            if (i + 12 + offset_count * UINT64_C(4) > moov_atom_size) {
                ERROR(" bad atom size/element count");
                goto error_out;
            }
    ...
    if (!copy_buffer) {
        ERROR("could not allocate %d bytes for copy_buffer", bytes_to_copy);
        goto error_out;
    }
    INFO(" copying rest of file...");
    while (last_offset) {
