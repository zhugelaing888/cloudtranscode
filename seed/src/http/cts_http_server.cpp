...
int HttpEpollWatcher::handle_request(Request &req, Response &res) {
	...
	if(this->resource_map.find(uri) == this->resource_map.end()) { // not found
		...
		INFO("page not found which uri:%s", uri.c_str());
		return 0;
	}
	...
	if(method.name != req.line.method) {
		...
		res.body.clear();
		INFO("not allow method, allowed:%s, request method:%s", method.name.c_str(), req.line.method.c_str());
		return 0;
	}
  ...
  DEBUG("handle response success which code:%d, msg:%s", res.code_msg.status_code, res.code_msg.msg.c_str());
	return 0;
}
...
int HttpEpollWatcher::on_writeable(EpollContext &epoll_context) {
	...
	DEBUG("send complete which write_num:%d, read_size:%d", nwrite, read_size);

	bool print_access_log = true;

	if (ret == 1) {/* not send over*/
	    print_access_log = false;
	    DEBUG("has big response, we will send part first and send other part later ...");
